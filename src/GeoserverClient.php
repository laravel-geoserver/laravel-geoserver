...
use LaravelGeoserver\LaravelGeoserver\Exceptions\DatastoreNotFoundException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\ErrorCreatingStyleException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\FeatureTypeNotFoundException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\GeomColumnNotFoundException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\NoContentInStyleException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\StyleContentNotFoundException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\StyleNotFoundException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\TableNotFoundException;
use LaravelGeoserver\LaravelGeoserver\Exceptions\WorkspaceNotFoundException;
...
class GeoserverClient
{
    ...
    {
        return Http::withBasicAuth(self::$username, self::$password)
            ->acceptJson()
            ->get(self::$baseUri.'rest/fonts.json')
            ->throw()
            ->json()['fonts'];
    }
    ...
    public static function featureTypeExists(string $workspaceName, string $datastoreName, string $featureTypeName)
    {
        ...
        return ! Str::of($body)->contains('No such');
    }
    ...
    public static function saveFeatureType(PostGisLayer $layer)
    {
        ...
        if ($layer->defaultStyle) {
            $layer->defaultStyle->isSaved = false;
            $layer->defaultStyle = $layer->defaultStyle->save();
            if ($layer->workspace) {
                $url = self::$baseUri.'rest/layers/'.$layer->workspace->name.':'.$layer->name;
            } else {
                $url = self::$baseUri.'rest/layers/'.$layer->name;
            }

            if ($layer->defaultStyle->workspace) {
                $style_name = $layer->defaultStyle->workspace->name.':'.$layer->defaultStyle->name;
            } else {
                $style_name = $layer->defaultStyle->name;
            }

            Http::withBasicAuth(self::$username, self::$password)
                ->withBody('<layer><defaultStyle><name>'.$style_name.'</name></defaultStyle><enabled>true</enabled></layer>', 'text/xml')
                ->withHeaders([
    ...
    public static function saveStyle(Style $style)
    {
        ...
        if (! $style->styleContent) {
            throw new NoContentInStyleException();
        }

        if (! $style->isSaved) {
            if ($style->workspace) {
                $workspaceName = $style->workspace->name;
                $url = self::$baseUri.'rest/workspaces/'.$style->workspace->name.'/styles';
            } else {
                $url = self::$baseUri.'rest/styles';
            }

            if (! self::styleExists($style->name, $workspaceName)) {
                $body = Http::withBasicAuth(self::$username, self::$password)
                    ->withBody('<style><name>'.$style->name.'</name><filename>'.$style->name.'.sld</filename></style>', 'text/xml')
                    ->withHeaders([
                        'Content-type' => 'text/xml',
                    ])
                    ->post($url)
                ...
                if ($body != $style->name) {
                    throw new ErrorCreatingStyleException($body);
                }
            }

            Http::withBasicAuth(self::$username, self::$password)
                ->withBody($style->styleContent, 'application/vnd.ogc.sld+xml')
                ->put($url.'/'.$style->name)
                ->throw();
        }

        return self::style($style->name, $workspaceName);
    }
        ...
        if (! is_null($response)) {
            if (isset($response->style)) {
                if ($response->style->name) {
                    if ($workspaceName) {
                        $style = Style::create($response->style->name, $workspaceName, true);
                    } else {
                        $style = Style::create($response->style->name, null, true);
                    }
                }
                if ($workspaceName) {
                    $style->workspace = self::workspace($workspaceName);
                }
    ...
    public static function styleExists(string $styleName, string $workspaceName = null)
    {
        try {
            $style = GeoserverClient::style($styleName, $workspaceName);
    ...
    public static function deleteStyle(string $styleName, string $workspaceName = null)
    {
        try {
            if (self::styleExists($styleName, $workspaceName)) {
                if ($workspaceName) {
                    $url = self::$baseUri.'rest/workspaces/'.$workspaceName.'/styles/'.$styleName;
                } else {
                    $url = self::$baseUri.'rest/styles/'.$styleName;
                }
